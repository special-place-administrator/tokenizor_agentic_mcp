---
goal_id: SFB19
title: Report deepest actionable validate_file_syntax parse errors
...
started_at: "2026-05-20T23:09:31.2379821+02:00"
completed_at: "2026-05-20T23:47:09.7782236+02:00"
completion_commit: "dbb70fd935fdd94919abdb3083cbb3d71f98b2a9"
blocked_reason: ""
gate: "implementation-ready"
risk_level: "medium"
source_refs:
  - "docs/live-code-backlog.md#16"
---
...
```text
/goal .agent/goals/symforge-live-code-backlog/SFB19-report-deepest-actionable-validate-file-syntax-parse-errors.md
```
...
objective:
- Change tree-sitter syntax diagnostics from first-error selection to deepest useful ERROR or MISSING node while preserving stable line/column/byte-span reporting.

non_goals:
- Do not change parser grammar dependencies.
- Do not change config-language diagnostics unless the new location is strictly more specific.
- Do not change validation tool name or input schema.
...
- Deepest useful ERROR/MISSING node is selected over shallow outer errors.
- Reported line, column, and byte span remain stable and deterministic.
- Config-language diagnostics remain unchanged unless demonstrably improved.

invariants:
- No syntax validator reports false success for malformed input.
- Existing validate_file_syntax tests continue to pass or are updated only for more specific locations.

acceptance_criteria:
- Malformed-source fixtures prove deepest nested error is selected.
- Existing config-language fixtures are unchanged or justify stricter location.
- Tests pin line/column/byte-span.

evidence_required:
- Before/after diagnostic samples.
- Test output.
- Default verification output.

stop_conditions:
- Tree-sitter node traversal cannot identify a stable deepest node for a fixture; use deterministic tie-breaking or block.
- New diagnostics fluctuate by parser version; pin fixture expectations or block.
...
rg "validate_file_syntax|ERROR|MISSING|deepest" src tests
```
