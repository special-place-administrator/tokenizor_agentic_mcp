---
goal_id: SFB16
title: Close co-change rerank calibration for weak anchors and chore anchors
chain_id: symforge-live-code-backlog
phase: Phase 3 - ranking and guidance quality
status: "Completed"
depends_on: []
...
started_at: "2026-05-20T21:56:14.0933016+02:00"
completed_at: "2026-05-20T22:10:41.0645261+02:00"
completion_commit: "d2b5de693632d7a0bfbb47ff01fb20c53b7c1c32"
blocked_reason: ""
gate: "decision-gated"
risk_level: "medium"
source_refs:
  - "docs/live-code-backlog.md#13"
---
# SFB16 - Close co-change rerank calibration for weak anchors and chore anchors

Use this file directly with `/goal`:

```text
/goal .agent/goals/symforge-live-code-backlog/SFB16-close-co-change-rerank-calibration-for-weak-anchors-and-chore-anchors.md
```
...
objective:
- Add a query-level calibration/regression corpus for `search_files(rank_by="path+cochange", anchor_path=...)`, prove weak anchors do not degrade baseline path ordering, and decide whether chore-anchor handling stays hardcoded or becomes workspace-configurable.

non_goals:
- Do not redesign frecency ranking.
- Do not remove path ranking fallback.
...
- Weak anchors must not degrade baseline path ordering.
- Chosen basename-tier threshold and chore-anchor behavior are pinned by tests.
- If configurable, configuration must have safe defaults and explicit status evidence.

invariants:
- Path-only ranking remains the fallback when co-change is unavailable/stale/disabled.
- Existing capability status evidence remains accurate.

acceptance_criteria:
- Regression corpus covers weak-anchor behavior and chore-anchor behavior.
- Chosen constants/config are documented in test names or code comments.
- Capability status tests still pass.

evidence_required:
- Decision hardcoded vs configurable.
- Test output from ranking/capability suites.
- Before/after ranking samples.
- Default verification output.

stop_conditions:
- Calibration corpus cannot be deterministic due git history dependence; use synthetic temp repo fixtures or block.
- Configurable chore anchors require a larger config-trust decision; keep hardcoded and document.
...
rg "chore|basename|anchor|path\+cochange|co-change" src tests
```
