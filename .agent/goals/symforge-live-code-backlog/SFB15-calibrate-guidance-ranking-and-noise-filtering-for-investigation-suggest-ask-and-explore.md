---
goal_id: SFB15
title: Calibrate guidance ranking and noise filtering for investigation_suggest, ask, and explore
...
started_at: "2026-05-20T21:32:32.8948729+02:00"
completed_at: "2026-05-20T21:52:13.6700120+02:00"
completion_commit: "2c907ce54924d097987736e2ffc1c21f5513921a"
blocked_reason: ""
gate: "evidence-gated"
risk_level: "medium"
source_refs:
  - "docs/live-code-backlog.md#12"
---
...
```text
/goal .agent/goals/symforge-live-code-backlog/SFB15-calibrate-guidance-ranking-and-noise-filtering-for-investigation-suggest-ask-and-explore.md
```
...
objective:
- Improve guidance tools so low-signal symbols, builtins/common names, and doc/comment-only pattern hits are filtered unless strongly contextualized, while suggestions include concise reason text.

non_goals:
- Do not rewrite all ranking infrastructure.
- Do not make outputs verbose.
- Do not change search_files co-change calibration in this goal.
...
- src/live_index/coupling/** except read-only ranking helper usage
- src/daemon.rs
- npm/**
- docs/**
- plans/**
...
- Prefer project-owned symbols, changed files, loaded-context proximity, caller/reference depth, and explicit reason text.
- Trivial names are filtered unless they have strong local context.
- Outputs remain concise.

invariants:
- No loss of relevant high-signal suggestions in common cases.
- Existing guidance tools remain available under the same public names.

acceptance_criteria:
- Focused tests prove trivial names are filtered unless strongly contextualized.
- At least one guidance response includes a concise reason for the suggestion.
- Existing ask/explore/investigation_suggest tests still pass.

evidence_required:
- Before/after sample guidance output.
- New test names and output.
- Default verification output.

stop_conditions:
- Ranking lacks inputs needed to prove project ownership/context; create a small metadata task instead.
- New filter hides high-signal project-owned symbols; revert or narrow.
...
rg "investigation_suggest|explore|reason|low-signal|low_signal" src tests
```
