---
goal_id: SFB10
title: Apply result-status semantics to read, search, and reference tools
...
started_at: "2026-05-20T14:40:39.2214133+02:00"
completed_at: "2026-05-20T15:26:17.1425475+02:00"
completion_commit: "ee2207eca5a20f6c9a5241dc36aa58c2073fc3e7"
blocked_reason: ""
...
```text
/goal .agent/goals/symforge-live-code-backlog/SFB10-apply-result-status-semantics-to-read-search-and-reference-tools.md
```
...
objective:
- Apply the result-status contract to `get_symbol`, `get_file_content`, `search_*`, and `find_references` while preserving existing human-readable output.

non_goals:
- Do not modify edit tools in this goal.
- Do not change ranking algorithms.
- Do not change file content bytes or symbol spans.
...
forbidden_files:
- src/protocol/edit.rs except shared result-status imports
- src/sidecar/**
- src/daemon.rs except proxy tests if required
- npm/**
- docs/**
- plans/**
- .planning/**
...
- Each targeted tool reports found/not_found/empty_result/ambiguous/invalid_request where applicable.
- Human-readable text remains understandable and compatible.
- No match and empty indexed result are distinct when the tool can distinguish them.

invariants:
- Search ranking and ordering remain unchanged unless status footer requires deterministic append only.
- Read tools never serve stale byte ranges silently.

acceptance_criteria:
- Contract tests cover found, not found, ambiguous selector, invalid request, and empty/no-match states across read/search/reference surfaces.
- Existing read/search/reference tests still pass.
- Response examples include parseable status evidence.

evidence_required:
- Test output for each targeted tool family.
- Before/after sample for one found and one not-found response.
- Default verification output.

stop_conditions:
- SFB09 artifacts are absent or incomplete.
- A tool cannot distinguish required states without invasive LiveIndex changes; record the gap and scope a separate goal.
