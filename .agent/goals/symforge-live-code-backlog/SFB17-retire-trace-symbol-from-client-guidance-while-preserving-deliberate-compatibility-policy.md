---
goal_id: SFB17
title: Retire trace_symbol from client guidance while preserving deliberate compatibility policy
...
started_at: "2026-05-20T22:18:45.1278913+02:00"
completed_at: "2026-05-20T22:37:58.1034774+02:00"
completion_commit: "2e8900f"
blocked_reason: ""
gate: "decision-gated"
risk_level: "medium"
source_refs:
  - "docs/live-code-backlog.md#14"
---
# SFB17 - Retire trace_symbol from client guidance while preserving deliberate compatibility policy

Use this file directly with `/goal`:

```text
/goal .agent/goals/symforge-live-code-backlog/SFB17-retire-trace-symbol-from-client-guidance-while-preserving-deliberate-compatibility-policy.md
```
...
objective:
- Remove `trace_symbol` from generated client allow lists and default tool-name guidance, then decide whether daemon compatibility keeps one final deprecation route or removes the alias now.

non_goals:
- Do not remove `find_references` or `get_symbol_context`.
- Do not rename public replacement tools.
- Do not change unrelated client initialization.
...
- src/daemon.rs
- src/cli/init.rs
- npm/** if generated client setup or allow lists include tool names
- tests/**
- README.md only if it still documents the retired alias
- CHANGELOG.md only if public compatibility policy changes

forbidden_files:
- src/live_index/** except tests that assert alias behavior
- src/protocol/edit.rs
- docs/** except README/CHANGELOG as above
- plans/**
- .planning/**
- openspec/**

contracts_or_interfaces:
- Client allow lists and default guidance no longer grant or recommend `trace_symbol`.
- Replacement paths are `find_references` and `get_symbol_context`.
- Compatibility route decision is explicit: KEEP_WITH_DEPRECATION or REMOVE_NOW.

invariants:
- Client init tests still pass.
- If compatibility route remains, it emits an explicit deprecation warning.
- No replacement tool behavior changes.

acceptance_criteria:
- `rg "trace_symbol"` returns only deliberate historical/compatibility references or none according to policy.
- Client init tests do not grant retired tool by default.
- Daemon compatibility behavior is tested if retained.

evidence_required:
- Compatibility decision.
- `rg "trace_symbol"` output.
- Client init and daemon tests.
- Default verification and npm test if npm changed.

stop_conditions:
- External client config still requires `trace_symbol` for current release; keep route with deprecation and document.
- Removing alias breaks schema roundtrip unexpectedly; block and decide compatibility window.
