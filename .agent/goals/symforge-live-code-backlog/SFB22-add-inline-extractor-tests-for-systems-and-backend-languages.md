...
```text
/goal .agent/goals/symforge-live-code-backlog/SFB22-add-inline-extractor-tests-for-systems-and-backend-languages.md
```
...
objective:
- Add minimal inline extractor coverage for Go, Java, C, C++, C#, and Swift extractors.

non_goals:
- Do not refactor parsers.
- Do not add broad grammar coverage.
- Do not upgrade parser crates.
...
- Unsupported or absent extractor cases are documented as concrete follow-up, not silently skipped.

invariants:
- No parser behavior changes unless required by a small fixture bug fix.
- Existing extractor tests remain passing.

acceptance_criteria:
- Go, Java, C, C++, C#, and Swift each have at least one inline test or a documented blocker.
- `cargo test --lib parsing -- --test-threads=1` passes.

evidence_required:
- List of languages covered/blockers.
- Parser test output.
- Default verification output.

stop_conditions:
- A fixture requires broad parser redesign; block that language and continue with remaining allowed languages.
...
rg "inline_test!|go|java|cpp|csharp|swift" src/parsing tests
```
