---
goal_id: SFB12
title: Surface runtime project and index identity in status output
...
started_at: "2026-05-20T20:00:32.3314658+02:00"
completed_at: "2026-05-20T20:25:30.9642269+02:00"
completion_commit: "9abdac0095c3a058c1d18ea96d464d9f8298c529"
blocked_reason: ""
...
```text
/goal .agent/goals/symforge-live-code-backlog/SFB12-surface-runtime-project-and-index-identity-in-status-output.md
```
...
objective:
- Make fresh process, reused daemon session, and explicit `index_folder` reset distinguishable by surfacing active project root, index/session identity, and fresh-vs-reused state in health or a dedicated status surface.

non_goals:
- Do not change daemon session lifecycle.
- Do not add analytics storage.
- Do not reset indexes automatically.
...
- npm/** unless client schema tests prove impacted
- docs/**
- plans/**
- .planning/**
- openspec/**
...
- Status output identifies active project root.
- Status output identifies session/index identity in a stable enough form for evaluation harness assertions.
- Status output reports whether index state is freshly built, snapshot-loaded/reused, daemon-reused, or reset by `index_folder`.

invariants:
- No secrets or absolute paths beyond already-disclosed project root policy are exposed unexpectedly.
- Health remains readable and compact health remains compact.
- Daemon proxy mode and local mode both remain supported.

acceptance_criteria:
- Tests distinguish fresh local process, reused daemon session, and explicit `index_folder` reset where supported.
- Health or status output includes project root and index/session identity.
- Evaluation harness can assert identity from output.
...
stop_conditions:
- Index identity requires persistent IDs that do not yet exist; stop and define minimal ID generation first.
- Daemon/local mode status diverges too much for one goal; implement local mode and block daemon mode with evidence.
...
rg "session_id|project_id|index.*identity|load_source|health_compact" src tests
```
